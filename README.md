...
11)  In the `script.js`, when a button is clicked, also change the alt text so that it contains the new position. At the end of the `advanceBlue` function, use `.alt` to set alt text. Here's [a resource with examples](https://www.w3schools.com/jsref/prop_img_alt.asp). Again, make sure you do this for all three horses.
...
```  
Check out this [resource on aria-live](https://developer.mozilla.org/en-US/docs/Web/Accessibility/ARIA/Reference/Attributes/aria-live) and this one on [aria-atomic](https://developer.mozilla.org/en-US/docs/Web/Accessibility/ARIA/Reference/Attributes/aria-atomic).
